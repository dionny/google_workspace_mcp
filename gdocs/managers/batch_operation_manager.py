...
class BatchOperationResult:
    ...
    The result distinguishes between two phases:
    1. Resolution: Converting search/location-based operations to index-based (resolved=True/False)
    2. Execution: Actually applying the operation to the document (executed=True/False)

    An operation can be resolved but not executed (e.g., if another operation in the batch
...
class BatchOperationManager:
    ...
    async def execute_batch_with_search(
        ...
        # Check if any resolution failed (resolved=False means resolution failed)
        failed_results = [r for r in operation_results if not r.resolved]
        if failed_results:
            return BatchExecutionResult(
                success=False,
                operations_completed=0,
        ...
        try:
            ...
            # Mark all operations as successfully executed
