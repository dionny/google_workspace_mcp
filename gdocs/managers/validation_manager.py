...
class ValidationManager:
    ...
    def validate_text_formatting_params(
        ...
        if all(param is None for param in formatting_params):
            return (
                False,
                "At least one formatting parameter must be provided (bold, italic, underline, strikethrough, small_caps, subscript, superscript, font_size, font_family, link, foreground_color, or background_color)",
            )
        ...
        if subscript and superscript:
            return (
                False,
                "subscript and superscript are mutually exclusive - only one can be True at a time",
            )
        ...
        if link is not None:
            ...
            if link and not (
                ...
                return (
                    False,
                    f"link must be a valid URL starting with http://, https://, or # (for internal bookmarks), got '{link}'",
                )
    ...
    def _validate_color_format(
        self, color: str, param_name: str = "color"
    ) -> Tuple[bool, str]:
        """
        ...
        if color.startswith("#"):
            ...
            return (
                False,
                f"Invalid hex color for {param_name}: '{color}'. Use #RGB (e.g., #F00) or #RRGGBB (e.g., #FF0000) format.",
            )

        # Not a valid format
        return (
            False,
            f"Invalid color format for {param_name}: '{color}'. Use hex (#FF0000, #F00) or named colors (red, blue, green, etc.).",
        )
    ...
    def validate_index(self, index: int, context: str = "Index") -> Tuple[bool, str]:
        ...
        if index < 0:
            return (
                False,
                f"{context} {index} is negative. You MUST call get_doc_info first to get the proper insertion index.",
            )
    ...
    def validate_document_id_structured(
    ...
    def validate_table_data_structured(
        ...
        for row_idx, row in enumerate(table_data):
            ...
            if len(row) == 0:
                error = DocsErrorBuilder.invalid_table_data(
                    f"Row {row_idx} is empty", row_index=row_idx
                )
                return False, format_error(error)
    ...
    def create_search_not_found_error(
        ...
        error = DocsErrorBuilder.search_text_not_found(
            search_text=search_text, similar_found=similar_found, match_case=match_case
        )
        return format_error(error)

    def create_ambiguous_search_error(
        self, search_text: str, occurrences: List[Dict[str, Any]], total_count: int
    ) -> str:
        """Create a structured error for ambiguous search results."""
        error = DocsErrorBuilder.ambiguous_search(
            search_text=search_text, occurrences=occurrences, total_count=total_count
        )
        return format_error(error)

    def create_heading_not_found_error(
        self, heading: str, available_headings: List[str], match_case: bool = True
    ) -> str:
    ...
    def create_invalid_occurrence_error(
        self, occurrence: int, total_found: int, search_text: str
    ) -> str:
        """Create a structured error for invalid occurrence number."""
        error = DocsErrorBuilder.invalid_occurrence(
            occurrence=occurrence, total_found=total_found, search_text=search_text
        )
        return format_error(error)

    def create_table_not_found_error(self, table_index: int, total_tables: int) -> str:
        """Create a structured error for table not found."""
        error = DocsErrorBuilder.table_not_found(
            table_index=table_index, total_tables=total_tables
        )
        return format_error(error)

    def create_missing_param_error(
        self, param_name: str, context: str, valid_values: Optional[List[str]] = None
    ) -> str:
    ...
    def create_invalid_param_error(
        self, param_name: str, received: Any, valid_values: List[str], context: str = ""
    ) -> str:
    ...
    def create_invalid_color_error(
        self, color_value: str, param_name: str = "color"
    ) -> str:
        """Create a structured error for invalid color format."""
        error = DocsErrorBuilder.invalid_color_format(
            color_value=color_value, param_name=param_name
        )
        return format_error(error)

    def create_api_error(
        self, operation: str, error_message: str, document_id: Optional[str] = None
    ) -> str:
        """Create a structured error for API failures."""
        error = DocsErrorBuilder.api_error(
            operation=operation, error_message=error_message, document_id=document_id
        )
        return format_error(error)
    ...
    def create_pdf_export_error(
        self, document_id: str, stage: str, error_detail: str
    ) -> str:
        """Create a structured error for PDF export failures."""
        error = DocsErrorBuilder.pdf_export_error(
            document_id=document_id, stage=stage, error_detail=error_detail
        )
        return format_error(error)

    def create_invalid_document_type_error(
        self, document_id: str, file_name: str, actual_mime_type: str
    ) -> str:
    ...
    def validate_index_in_bounds(
        self, index: int, doc_length: int, index_name: str = "index"
    ) -> Tuple[bool, Optional[str]]:
        """
        ...
        # Check upper bound
        if index >= doc_length:
            error = DocsErrorBuilder.index_out_of_bounds(index_name, index, doc_length)
            return False, format_error(error)

        return True, None

    def validate_mutually_exclusive(
        self, params: Dict[str, Any], exclusive_groups: List[List[str]]
    ) -> Tuple[bool, Optional[str]]:
        """
