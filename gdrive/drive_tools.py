...
async def delete_drive_file(
    ...
    """
    Deletes a file from Google Drive. By default, moves the file to trash (recoverable).
    Use permanent=True to permanently delete (irreversible).

    Args:
        user_google_email (str): The user's Google email address. Required.
        file_id (str): The ID of the file to delete. Required.
        permanent (bool): If True, permanently deletes the file (cannot be recovered).
                         If False (default), moves the file to trash.

    Returns:
        str: Confirmation message of the deletion.
    """
    logger.info(
        f"[delete_drive_file] Deleting file {file_id} for {user_google_email}, permanent={permanent}"
    )
    ...
    if permanent:
        ...
        return f"Permanently deleted file '{file_name}' (ID: {file_id}) for {user_google_email}. This action cannot be undone."
    else:
        ...
        return f"Moved file '{file_name}' (ID: {file_id}) to trash for {user_google_email}. The file can be recovered from the trash."
