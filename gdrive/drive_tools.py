...
async def delete_drive_file(
    ...
    By default, moves the file to trash (recoverable). Set permanent=True to permanently delete.

    Args:
        user_google_email (str): The user's Google email address. Required.
        file_id (str): The ID of the file to delete. Required.
        permanent (bool): If True, permanently deletes the file. If False (default), moves to trash.

    Returns:
        str: Confirmation message of the deletion.
    """
    logger.info(
        f"[delete_drive_file] Invoked. Email: '{user_google_email}', File ID: '{file_id}', Permanent: {permanent}"
    )
    ...
    if permanent:
        ...
        return f"Permanently deleted file '{file_name}' (ID: {file_id}) for {user_google_email}."
    else:
        await asyncio.to_thread(
            service.files()
        ...
        return f"Moved file '{file_name}' (ID: {file_id}) to trash for {user_google_email}. Use permanent=True to delete permanently."
