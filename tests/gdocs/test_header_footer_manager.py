...
class TestReplaceSectionContent:
    ...
    @pytest.mark.asyncio
    async def test_replace_content_in_section_with_text(
        self, manager, mock_batch_update_success
    ):
        ...
        result = await manager._replace_section_content(
            "doc123", section, "New Header", "kix.header123"
        )
        ...
        assert (
            requests[0]["deleteContentRange"]["range"]["segmentId"] == "kix.header123"
        )
    ...
    @pytest.mark.asyncio
    async def test_replace_content_in_empty_section_with_paragraph(
        self, manager, mock_batch_update_success
    ):
    ...
    @pytest.mark.asyncio
    async def test_replace_content_when_indices_equal(
        self, manager, mock_batch_update_success
    ):
        """Test when start_index equals end_index (truly empty)."""
        section = {
            "content": [{"paragraph": {"elements": []}, "startIndex": 0, "endIndex": 0}]
        }
    ...
    @pytest.mark.asyncio
    async def test_replace_content_with_empty_content_list(
        self, manager, mock_batch_update_success
    ):
    ...
    @pytest.mark.asyncio
    async def test_replace_content_no_content_key(
        self, manager, mock_batch_update_success
    ):
    ...
    async def test_api_error_returns_false(self, manager):
        ...
        manager.service.documents.return_value.batchUpdate.return_value.execute = (
            mock_execute
        )

        section = {"content": [{"paragraph": {}, "startIndex": 0, "endIndex": 1}]}
    ...
    @pytest.mark.asyncio
    async def test_delete_range_not_created_when_would_be_empty(
        self, manager, mock_batch_update_success
    ):
        ...
        result = await manager._replace_section_content(
            "doc123", section, "Test", "kix.header123"
        )
        ...
        for req in requests:
            assert "deleteContentRange" not in req, (
                "Should not delete when range would be empty"
            )
