...
def create_mock_document(elements, inline_objects=None):
    ...
    doc = {"title": "Test Document", "body": {"content": elements}, "lists": {}}
    if inline_objects:
...
class TestExtractImagesHelper:
    ...
    def test_finds_images_in_document(self):
        ...
        assert (
            props1["imageProperties"]["contentUri"]
            ...
            if "paragraph" in element:
                ...
                for para_elem in paragraph.get("elements", []):
                    if "textRun" in para_elem:
                        ...
                        is_monospace = any(
                            mono in font_family for mono in MONOSPACE_FONTS
                        )
