...
async def test_document(user_google_email, docs_service, drive_service):
    ...
    result = await docs_tools_module.create_doc.fn(
        user_google_email=user_google_email, title=title
    )
    ...
    # Yield to the test
    yield {"document_id": document_id, "title": title}

    # Cleanup: Always trash the document, even if test failed
    try:
        await drive_tools_module.update_drive_file.fn(
            user_google_email=user_google_email, file_id=document_id, trashed=True
        )
...
async def populated_test_document(user_google_email, docs_service, drive_service):
    ...
    structure = {"heading_positions": {}, "list_positions": {}, "table_positions": {}}

    yield {"document_id": document_id, "title": title, "structure": structure}
...
async def batch_update(
    docs_service, document_id: str, requests: list
) -> Dict[str, Any]:
    """
    ...
    return await asyncio.to_thread(
        docs_service.documents().batchUpdate(documentId=document_id, body=body).execute
    )
