...
async def test_search_text_simple(user_google_email, test_document):
    ...
    assert result_data.get('success') or result_data.get('modified'), f"Should find and insert after text: {result}"
...
async def test_insert_before_searched_text(user_google_email, test_document):
    ...
    result_data = json.loads(result)
    assert result_data.get('success') or result_data.get('modified'), f"Should insert before: {result}"
...
async def test_replace_searched_text(user_google_email, test_document):
    ...
    result_data = json.loads(result)
    assert result_data.get('success') or result_data.get('modified'), f"Should replace text: {result}"
...
async def test_insert_at_searched_location(user_google_email, test_document):
    ...
    result_data = json.loads(result)
    assert result_data.get('success') or result_data.get('modified'), f"Should replace marker: {result}"
