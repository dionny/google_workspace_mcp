...
async def test_insert_text_at_start(user_google_email, test_document):
    ...
    assert result_data.get("success") or result_data.get("modified"), (
        f"Insert should succeed: {result}"
    )


@pytest.mark.asyncio
@pytest.mark.integration
async def test_create_simple_bullet_list(
    user_google_email, docs_service, test_document
):
...
async def test_apply_bold_formatting(user_google_email, docs_service, test_document):
    ...
    result_data = json.loads(result)
    assert result_data.get("success") or result_data.get("modified"), (
        f"Format should succeed: {result}"
    )
