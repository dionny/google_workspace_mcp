...
async def test_create_heading_h1(user_google_email, test_document):
    ...
    assert result_data.get('success') or result_data.get('modified'), f"Should create H1: {result}"
...
async def test_create_heading_h2(user_google_email, test_document):
    ...
    result_data = json.loads(result)
    assert result_data.get('success') or result_data.get('modified'), f"Should create H2: {result}"
...
async def test_create_heading_h3(user_google_email, test_document):
    ...
    result_data = json.loads(result)
    assert result_data.get('success') or result_data.get('modified'), f"Should create H3: {result}"
...
async def test_multiple_headings_structure(user_google_email, test_document):
    ...
    result_data = json.loads(result)
    assert result_data.get('success') or result_data.get('modified'), f"Should create structure: {result}"
...
async def test_heading_with_content(user_google_email, test_document):
    ...
    result_data = json.loads(result)
    assert result_data.get('success') or result_data.get('modified'), f"Should add content: {result}"
