...
class CalendarFeatureTests:
    ...
    def __init__(self, email: str):
        ...
        if days_until_monday == 0:
            days_until_monday = 7
        self.next_monday = (today + timedelta(days=days_until_monday)).strftime(
            "%Y-%m-%d"
        )
        self.next_week = (today + timedelta(days=days_until_monday + 7)).strftime(
            "%Y-%m-%d"
        )
    ...
    async def test_2_1_weekday_grouping(self):
        ...
        result = await self.call_tool(
            "get_events_times_only", time_min=self.next_monday, time_max=self.next_week
        )
        passed = any(
            day in result
    ...
    async def test_2_2_recurring_dedup(self):
        ...
        two_weeks = (
            datetime.strptime(self.next_monday, "%Y-%m-%d") + timedelta(days=14)
        ).strftime("%Y-%m-%d")
        result = await self.call_tool(
    ...
    async def test_2_3_allday_format(self):
        ...
        next_day = (
            datetime.strptime(self.next_monday, "%Y-%m-%d") + timedelta(days=1)
        ).strftime("%Y-%m-%d")
    ...
    async def test_3_3_delete_event(self):
        ...
        if match:
            ...
            self.add_result(
                "3.3 Delete Event", False, "No test event found", warning=True
            )
