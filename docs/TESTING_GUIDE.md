...
python tools_cli.py --info get_doc_content

# Call a tool
python tools_cli.py --tool search_docs \
  --query "test" \
  --user_google_email "user@example.com" \
  --page_size 5

# Interactive mode
python tools_cli.py --interactive
```
...
Use the tools CLI in interactive mode:
...
### "Tool not found" (tools CLI)

The tools CLI loads tools dynamically. Use `--verbose` to see what's happening:
```bash
python tools_cli.py --list --verbose
```
