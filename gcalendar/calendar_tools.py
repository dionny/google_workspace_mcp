...
async def get_events_times_only(
    ...
    for item in items:
        ...
        try:
            ...
            event_key = (
                f"{summary} ({start_time}-{end_time})"
                if end_time
                else f"{summary} ({start_time})"
            )
    ...
    for weekday in [
        ...
        if events:
            ...
            for event_key, count in events.items():
                if count > 1:
                    output_lines.append(
                        f"  - {event_key} (recurring, {count} occurrences)"
                    )
                else:
    ...
    logger.info(
        f"[get_events_times_only] Successfully retrieved events for {user_google_email}"
    )
    return "\n".join(output_lines)
