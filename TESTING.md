...
You can test your MCP tools directly without running the full MCP server using the test harness.
...
```bash
uv run python test_harness.py --info search_docs
```

### Call a tool directly
```bash
uv run python test_harness.py --tool search_docs \
  --query "test" \
...
```bash
uv run python test_harness.py --interactive
...
- Use `--verbose` flag for debug logging: `uv run python test_harness.py --verbose --tool ...`
