...
class ToolTester:
    ...
    async def _collect_tools(self) -> Dict[str, Any]:
        ...
        if hasattr(self.server, "_tool_manager"):
            ...
            if hasattr(tool_manager, "_tools"):
                # FastMCP stores tools in _tools dict directly, no list_tools() method exists
    ...
    async def call_tool(self, tool_name: str, **kwargs) -> Any:
        ...
        if tool_name not in self.tools:
            raise ValueError(
                f"Tool '{tool_name}' not found. Use list_tools() to see available tools."
            )
    ...
    while True:
        try:
            ...
            if cmd.startswith("call "):
                ...
                print(
                    "❌ Unknown command. Try 'list', 'info <tool>', 'call <tool>', or 'quit'"
                )
...
def main():
    ...
    parser.add_argument(
        "--list", "-l", action="store_true", help="List all available tools"
    )
    parser.add_argument("--tool", "-t", type=str, help="Tool name to call")
    parser.add_argument("--info", type=str, help="Show detailed info about a tool")
    parser.add_argument(
        "--user_google_email",
        type=str,
    ...
    def unescape_shell_chars(value: str) -> str:
        ...
        When users pass arguments like --range_name 'Sheet1\!A1:B2' to prevent
        bash history expansion, the backslash may be preserved in the string.
        This function removes such escape characters.
        """
        if not isinstance(value, str):
            return value
        # Unescape common shell-escaped special characters
        # Order matters: handle double backslashes first
    ...
    def convert_value_by_type(value: str, expected_type) -> Any:
        ...
        if expected_type is bool:
            ...
            try:
                ...
                return unescape_shell_chars(value)
        else:
            # Default: keep as string, unescape shell characters
            return unescape_shell_chars(value)
    ...
    def convert_value_fallback(value: str) -> Any:
        ...
        # Unescape shell characters for string values
        return unescape_shell_chars(value)
    ...
    def parse_tool_kwargs(tool, raw_kwargs: dict) -> dict:
        ...
        for param_name, value in raw_kwargs.items():
            if param_name in type_hints:
                ...
                if param.annotation != inspect.Parameter.empty:
                    tool_kwargs[param_name] = convert_value_by_type(
    ...
    if args.tool:
        ...
        if args.tool not in tester.tools:
            print(
                f"❌ Tool '{args.tool}' not found. Use --list to see available tools."
            )
            sys.exit(1)
    ...
    print(
        "  python tools_cli.py --tool search_docs --query 'test' --user_google_email user@example.com"
    )
